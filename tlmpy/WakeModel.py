    ...
    for i in numba.prange(Nt):
        ...
        for k in numba.prange(Nt):
            ...
            for n in numba.prange(Nq):
                ...
                deficit,defjac_x,defjac_y = gauss_wakedeficit(delta_str,delta_span,zn,D[k],Ct[k],kwake,beta,eps)
                A[i,k] += wn*deficit
                #Compute A_jac
                ...
                dAdu[i,k] += wn*(defjac_x*dstr_jac[0]+defjac_y*dspan_jac[0])
                dAdv[i,k] += wn*(defjac_x*dstr_jac[1]+defjac_y*dspan_jac[1])
...
def gauss_wakedeficit(x,y,z,d0,Ct,kwake,beta,eps):
    ...
    -------
    deficit: float
        velocity deficit and 
    numpy array with size (2,):
    ...
    defdx  = (-Ct*kwake/(8*d0*sigma_d0**3)/np.sqrt(1-Ct/(8*sigma_d0**2))*
             np.exp(-1./(2*sigma_d0**2)*( (z/d0)**2+(y/d0)**2) ) +
             (deficit*kwake/(d0*sigma_d0**3)*( (z/d0)**2+(y/d0)**2) ) )
    defdy  = -deficit*y/(d0**2*sigma_d0**2)
    return deficit, defdx, defdy
...
def jensen(turbines,Uinf,Vinf,kwake):
    '''Jensen wake model (depreciated) - Jacobian not available'''
    Sinf = np.sqrt(Uinf**2+Vinf**2)   #Wind speed
    e_str = np.array([Uinf,Vinf])/Sinf #Unit vector along the wind direction
    e_span = np.array([-Vinf,Uinf])/Sinf #Unit vector in cross wind direction
