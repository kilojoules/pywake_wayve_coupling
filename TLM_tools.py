...
@njit(parallel=False)
def evaluate_A(U1,V1,U2,V2,S1,dU12,dV12,dS12,H1,H2,C0,C1,nu1,nu2,fc,Nx,Ny,N,
               u1,v1,u2,v2,p1,p2,ks,ls,PHI,F1u,F1v):
    '''
    ...
    for index in numba.prange(N):
        sigma1 = U1*ks[index]+V1*ls[index]
        sigma2 = U2*ks[index]+V2*ls[index]
        #Regular entries
        #u1 equation
        Axu1[index] = ( - 1j*sigma1*u1[index]
                        - 1j*ks[index]*(p1[index]+p2[index])
                        + fc*v1[index] 
                        ...
                        - nu1*(ks[index]**2+ls[index]**2)*u1[index]
                        - F1u[index]/H1 )
        ...
        Axv1[index] = ( - 1j*sigma1*v1[index]
                        ...
                        - nu1*(ks[index]**2+ls[index]**2)*v1[index] 
                        - F1v[index]/H1 )
        #u2 equation
        Axu2[index] = ( - 1j*sigma2*u2[index]
                        ...
                        - nu2*(ks[index]**2+ls[index]**2)*u2[index] )
        #v2 equation
        Axv2[index] = ( - 1j*sigma2*v2[index]
                        ...
                        - nu2*(ks[index]**2+ls[index]**2)*v2[index] )
        #p1 equation
        #General case
        Axp1[index] = ( + PHI[index]*H1*ks[index]*u1[index]
                        + PHI[index]*H1*ls[index]*v1[index]
                        + sigma1*p1[index] )
        #p2 equation
        #General case
        Axp2[index] = ( + PHI[index]*H2*ks[index]*u2[index]
...
@njit(parallel=True)
def evaluate_M(ks,ls,Ny,Nx,U1,V1,U2,V2,C0,C1,S1,H1,dS12,dU12,nu1,nu2,dV12,fc,H2,PHI,N,X):
    '''
    ...
    for indexk in numba.prange(ks.size):
        for indexl in numba.prange(ls.size):
            ...
            sigma1 = U1*ks[indexk]+V1*ls[indexl]
            sigma2 = U2*ks[indexk]+V2*ls[indexl]
            #u1 equation
            P[0,0] = -1j*sigma1-C0*(S1**2+U1**2)/(S1*H1)-C1*(dS12**2+dU12**2)/(dS12*H1)-nu1*(ks[indexk]**2+ls[indexl]**2) 
            ...
            P[1,1] = -1j*sigma1-C0*(S1**2+V1**2)/(S1*H1)-C1*(dS12**2+dV12**2)/(dS12*H1)-nu1*(ks[indexk]**2+ls[indexl]**2)
            ...
            P[2,2] = -1j*sigma2-C1*(dS12**2+dU12**2)/(dS12*H2)-nu2*(ks[indexk]**2+ls[indexl]**2) 
            ...
            P[3,3] = -1j*sigma2-C1*(dS12**2+dV12**2)/(dS12*H2)-nu2*(ks[indexk]**2+ls[indexl]**2)
            P[3,4] = -1j*ls[indexl]
            P[3,5] = -1j*ls[indexl]
            #p1 equation
            #General case
            P[4,0] = PHI[indexk,indexl]*H1*ks[indexk]
            P[4,1] = PHI[indexk,indexl]*H1*ls[indexl]
            P[4,4] = sigma1
            #p2 equation
            #General case
            P[5,2] = PHI[indexk,indexl]*H2*ks[indexk]
            P[5,3] = PHI[indexk,indexl]*H2*ls[indexl]
            P[5,5] = sigma2
            #k=l=0
            if ks[indexk]==0 and ls[indexl] == 0:
                P[4,4] = 1.+0.j
                P[5,5] = 1.+0.j
            #PHI=0
            ...
            #M[:,index] = np.linalg.solve(P,X[:,index])        #BEST - Use on SPC (set parallel=False)
            M[:,index] = np.dot(np.linalg.inv(P),X[:,index])   #BEST - Use on PC             
    return M

@njit(parallel=False)
def evaluate_PHI(gprime,shape,ks,ls,U3,V3,N):  
    '''
    ...
    for indexk in numba.prange(ks.size):
        for indexl in numba.prange(ls.size):
            sigma3 = U3*ks[indexk] + V3*ls[indexl]
            #Non-hydrostatic solution 
            if not sigma3==0:
                if sigma3**2>N**2:
                    m = 1j*np.sqrt((ks[indexk]**2+ls[indexl]**2)*np.abs(N**2/sigma3**2-1))
                else:
                    m = np.sign(sigma3)*np.sqrt((ks[indexk]**2+ls[indexl]**2)*(N**2/sigma3**2-1))
                PHI[indexk,indexl] += 1j/m*(N**2-sigma3**2)
            #Hydrostatic solution 
            #if not sigma3==0:
            #    m = np.sign(sigma3)*np.sqrt((k**2+l**2)*(N**2/sigma3**2))
